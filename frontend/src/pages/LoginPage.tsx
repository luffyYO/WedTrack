  ...
  const verifyOtp = async () => {
    ...
    if (error || !data.session) {
      ...
      if (error2 || !data2.session) {
        setError(error2?.message || error?.message || "Verification failed. Please try again.");
      }
      // ✅ If success: onAuthStateChange fires → authStore.setSession → AuthLayout redirects to /home
    } else {
      setLoading(false);
      // ✅ Session set via onAuthStateChange → AuthLayout redirects automatically
    }
  };
