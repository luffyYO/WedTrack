    ...
    return (
        <div className="grid grid-cols-2 gap-3 sm:gap-6 max-w-[850px] mx-auto z-10 relative px-4 sm:px-6">
            ...
            <div className="glass-panel p-3 sm:p-7 rounded-[1.5rem] sm:rounded-[2rem] flex flex-col items-center justify-center text-center gap-1 sm:gap-1.5 h-[120px] sm:h-[160px] hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_12px_40px_rgba(0,0,0,0.5)] hover:-translate-y-1 transition-all group overflow-hidden relative">
                ...
                <span className="text-slate-400 dark:text-[#6a7585] text-[8px] sm:text-xs font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] relative z-10 leading-tight">Validated Revenue</span>
                {/* Use solid color in dark mode — gradient is invisible on dark bg */}
                ...
                <span className="text-[7px] sm:text-[10px] text-pink-400/80 dark:text-pink-400 font-bold mt-0.5 sm:mt-1 relative z-10 hidden xs:block">SECURE TRANSACTION LEDGER</span>
            </div>

            {/* Total Gifts — pink gradient card is self-contained; no dark mode fix needed */}
            <div className="bg-gradient-to-br from-pink-500 to-rose-400 text-white p-3 sm:p-7 rounded-[1.5rem] sm:rounded-[2rem] shadow-[0_8px_30px_rgba(236,72,153,0.3)] border border-pink-400 flex flex-col items-center justify-center text-center gap-1 sm:gap-1.5 h-[120px] sm:h-[160px] hover:shadow-[0_12px_40px_rgba(236,72,153,0.4)] hover:-translate-y-1 transition-all group overflow-hidden relative">
                ...
                <span className="text-white/80 text-[8px] sm:text-xs font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] relative z-10 leading-tight">Verified Contributions</span>
                ...
                <span className="text-[7px] sm:text-[10px] text-white/70 font-bold relative z-10 mt-0.5 sm:mt-1 uppercase hidden xs:block">Confirmed Guest Entries</span>
            </div>
            ...
            <div className="glass-panel p-3 sm:p-6 rounded-[1.5rem] flex flex-col items-center justify-center text-center gap-1 h-[110px] sm:h-[140px] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] dark:hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] hover:-translate-y-0.5 transition-all group relative">
                <span className="text-slate-400 dark:text-[#6a7585] text-[8px] sm:text-[10px] font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] leading-tight">Total Inventory</span>
                <h3 className="text-2xl sm:text-4xl font-black text-slate-700 dark:text-[#e6edf3] mt-0.5 sm:mt-1 tracking-tight">
                    <span className={`transition-opacity duration-300 ${guestsLoading ? 'opacity-30' : 'opacity-100'}`}>
                        {totalGuests}
                    </span>
                </h3>
                <span className="text-[7px] sm:text-[9px] text-slate-400 dark:text-[#6a7585] font-bold mt-0.5">ALL SUBMISSIONS</span>
            </div>

            {/* Pending Verifications */}
            <div className="glass-panel p-3 sm:p-6 rounded-[1.5rem] border border-red-200 dark:border-red-900/50 flex flex-col items-center justify-center text-center gap-1 h-[110px] sm:h-[140px] hover:shadow-[0_8px_30px_rgba(239,68,68,0.15)] hover:border-red-300 dark:hover:border-red-800 transition-all group relative">
                {pendingGifts > 0 && (
                ...
                <span className="text-red-400 dark:text-red-400 text-[8px] sm:text-[10px] font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] leading-tight">Awaiting Action</span>
                <h3 className="text-2xl sm:text-4xl font-black text-red-500 dark:text-red-400 mt-0.5 sm:mt-1">
                    <span className={`transition-opacity duration-300 ${guestsLoading ? 'opacity-30' : 'opacity-100'}`}>
                        {pendingGifts}
                    </span>
                </h3>
                <span className="text-[7px] sm:text-[9px] text-red-400/80 dark:text-red-400/70 font-bold mt-0.5 uppercase">Verification Required</span>
            </div>
        </div>
    );
}
