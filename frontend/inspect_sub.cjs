/**
 * Inspect the full push subscription data including keys
 * to determine if it's a real VAPID endpoint or an FCM token
 */
...
async function main() {
  ...
  const result = await post('/functions/v1/dev-admin',
    { action: 'get_push_subscriptions_full' },
    { 'x-internal-key': DEV_ADMIN_TOKEN }
  );

  console.log('Status:', result.status);
  console.log('Body:', JSON.stringify(result.body, null, 2).substring(0, 2000));
}
